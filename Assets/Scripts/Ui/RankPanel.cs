    ...
    public void SetRankUI(List<User> userList)
    {
        ...
        for (int i = 0; i < 8; i++) //只显示前8名，前三名有奖牌，显示后边
        {
            ...
            if (userList.Count <= i)
            {
                ...
                rankItem.Find("UserIcon").GetChild(0).GetComponent<Image>().sprite = null;
            }
            else
            {
                var item = userList[i];
                rankItem.Find("Name").GetComponent<TMP_Text>().text = item.username;
                rankItem.Find("Data").GetComponent<TMP_Text>().text = $"贡献等级:{item.score}";
                rankItem.Find("UserIcon").GetChild(0).GetComponent<Image>().sprite = item.userIcon;
            }
        }
    }
