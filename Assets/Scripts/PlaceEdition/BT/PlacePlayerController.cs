...
public class PlacePlayerController : MonoBehaviour
{
    ...
    public IEnumerator DrawPattern(RefWrapper<bool> btDrawing, RefWrapper<bool> btDrawFinish)
    {
        int timer = 0;
        // insQueue 身上携带的指令
        while (insQueue.Count > 0)
        {
            Instruction ins = insQueue.Dequeue();
        ...
        Debug.Log($"[PC] : 跑一趟身上的指令数 : {batchInsCount} 跑一趟画的次数: {batchDrawTimes}");
        if (batchInsCount == batchDrawTimes)
        {
            // 画完了
            if (user.currentCarryingInkCount != 0)
            {
                Debug.Log($"还剩下 {user.currentCarryingInkCount} 颜料!");
            }
        }
