...
public class PlaceTeamInstructionManager : MonoBehaviour
{
    // 解析字符串，生成指令
    /*
    [创作绘画] 点线面
    ...
    */
    ...
    // 带有 指令 符号/
    private static PlaceTeamInstructionManager instance;
    public static PlaceTeamInstructionManager Instance
    {
        get
        {
            if (instance == null)
            {
                instance = FindObjectOfType<PlaceTeamInstructionManager>();
            }
            return instance;
        }
    }

    public PlaceTeamBoard team1Board;
    public PlaceTeamBoard team2Board;
        ...
        switch (parts[0])
        {
            ...
            case "9":
                ...
                if (parts.Length > 1)
                {
                    ...
                    if (!PlaceTeamBoardManager.Instance.CheckIns(drawIns))
                    {
                        Debug.Log("指令不合法");
            ...
            case "/l":
                if (parts.Length >= 5)
                {
                    if (parts.Length == 5)
                    {
                        ...
                        if (!PlaceTeamBoardManager.Instance.CheckIns(ins_l))
                        {
                        ...
                        {
                            // ======= Line2point =======
                            List<(int, int)> points = PlaceTeamBoardManager.Instance.GetLinePoints(x, y, ex, ey);
            ...
            case "/p":
                ...
                if (parts.Length >= 5)
                {
                    if (parts.Length == 5)
                    {
                        ...
                        if (!PlaceTeamBoardManager.Instance.CheckIns(ins_p))
                        {
                        ...
                        // ======= Square2point =======
                        List<(int, int)> points = PlaceTeamBoardManager.Instance.GetSquarePoints(x, y, dx, dy);
                        foreach ((int, int) point in points)
                        {
                            Instruction drawIns = new Instruction("/d", point.Item1, point.Item2, r: r, g: g, b: b);
                            user.instructionQueue.Enqueue(drawIns);
            ...
            case "/r":
                if (parts.Length >= 5)
                {
                    if (parts.Length == 5)
                    {
                        ...
                        if (!PlaceTeamBoardManager.Instance.CheckIns(ins_rect))
                        {
                        ...
                        if (user.level < 50)
                        {
                            List<(int, int)> points = PlaceTeamBoardManager.Instance.GetRectPoints(x, y, dx, dy);
                            foreach ((int, int) point in points)
                            {
                                Instruction drawIns = new Instruction("/d", point.Item1, point.Item2, r: r, g: g, b: b);
                                user.instructionQueue.Enqueue(drawIns);
                        ...
                        {
                            // ====== Rect2Line =======
                            List<(int, int, int, int)> lines = PlaceTeamBoardManager.Instance.GetRectLines(x, y, dx, dy);
            ...
            case "/c":
                if (parts.Length == 4)
                {
                    ...
                    // 检测 合法
                    if (x + r >= PlaceTeamBoardManager.Instance.width || y + r >= PlaceTeamBoardManager.Instance.height || x - r < 0 || y - r < 0)
                    {
                    ...
                    // ======= Square2point =======
                    List<(int, int)> points = PlaceTeamBoardManager.Instance.GetCirclePoints(x, y, r);
                    foreach ((int, int) point in points)
                    {
                ...
                else if (parts.Length == 5)
                {
                    ...
                    // 检测 合法
                    if (x + radius >= PlaceTeamBoardManager.Instance.width || y + radius >= PlaceTeamBoardManager.Instance.height || x - radius < 0 || y - radius < 0)
                    {
                    ...
                    // ======= Square2point =======
                    List<(int, int)> points = PlaceTeamBoardManager.Instance.GetCirclePoints(x, y, radius);
                    foreach ((int, int) point in points)
                    {
                        Instruction drawIns = new Instruction("/d", point.Item1, point.Item2, r: r, g: g, b: b);
                        user.lastColor = color;
            ...
            case "/m":
                if (parts.Length == 2) // 默认颜色
                {
                    ...
                    x = Mathf.Clamp(x, 0, PlaceTeamBoardManager.Instance.width - 1);
                    y = Mathf.Clamp(y, 0, PlaceTeamBoardManager.Instance.height - 1);
                    user.lastPoint = (x, y);
                }
            ...
            //     break; // 2024-05-09 :⚠️ 暂时废弃
                       ...
                       //         break;
                       //     }
                       //     List<(int, int)> points = PlaceTeamBoardManager.Instance.GetFillPoints(x, y, user.Id);
                       //     foreach ((int, int) point in points)
                       //     {
                       //         Instruction drawIns = new Instruction("/d", point.Item1, point.Item2, r: r, g: g, b: b);
                       //         user.instructionQueue.Enqueue(drawIns);
