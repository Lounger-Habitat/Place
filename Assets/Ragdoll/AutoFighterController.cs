using UnityEngine;
public class AutoFighterController : AutoCharacterController
{
    ...
    void FindTarget()
    {
        ...
        else
        {
            ...
            for (int i = 0; i < targets.Length; i++)
            {
                ...
                {
                    if (!targets[i].GetComponent<AutoHitController>().IsDead)
                    {
                        dist = dir.magnitude;
                        id = i;
                    }
            ...
            isFocused = id != -1;
            if (isFocused)
                ChangeTarget(targets[id].GetComponent<AutoHitController>().head);
            else
                ChangeTarget(null);
