...
public class ProGifManager : MonoBehaviour
{
	...
	private void _InitRecorder(Camera targetCamera, bool autoClear)
	{
		Clear();

		if(m_Fps > m_MaxFps) m_Fps = m_MaxFps;

		m_GifRecorder = new ProGifRecorder(targetCamera);
		...
		m_GifRecorder.SetTransparent(m_TransparentColor, m_TransparentColorRange);
        ...
        {
            m_GifRecorder.SetTransparentExtras(true, m_Width, m_Height);
        }
        else
        {
            m_GifRecorder.SetTransparentExtras(false);
        }
    ...
    public void SetTransparent(Color32 color32, byte transparentColorRange)
	{
		...
		m_GifRecorder.SetTransparent(color32, transparentColorRange);
	}
    ...
    /// <summary>
    /// Clear both the recorder and player handled by this manager.
    /// </summary>
	public void Clear()
	{
		ClearRecorder();
    ...
    public void ClearRecorder_Delay(Action onClear = null)
    {
        ...
        m_GifRecorder.recorderCom.OnFileSaved += (workerId, gifPath) => {
            isSaved = true;
        ...
        m_GifPlayer.playerComponent.OnLoading += (progress) => {
            if (progress >= 1)
            {
                isLoaded = true;
